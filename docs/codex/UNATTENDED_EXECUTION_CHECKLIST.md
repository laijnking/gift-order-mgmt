...
- [x] 页面适配尾差已继续推进三轮：`agent-configs / ai-logs / archive / roles / templates / users / orders / shipping-export / return-receipt / customers / suppliers-manage / products / export-records / alerts / warehouses-manage / reports / order-cost-history / stocks` 已补齐一批移动端按钮换行、表格横向滚动、对话框安全宽度和弹窗底部按钮堆叠；当前 `ts-check` 已通过，针对性 `eslint` 无新增错误，仅保留仓库原有 warning
