...
| `assigned` | 已派发 | partial_returned,notified , returned, cancelled |
| `notified` | 通知发货 | partial_returned, returned, cancelled |
| `partial_returned` | 部分回单 | returned, completed |
| `returned` | 已回单 | completed |
| `completed` | 已完成 | - |
| `cancelled` | 已取消 | - |

> `feedbacked` 不是独立状态，已并入"回单阶段"统计（聚合 assigned / partial_returned / returned / feedbacked 四种状态）。
...
| 阶段 | 条件 | 操作 |
|------|------|------|
| 新增 | 满足规则 + 无未处理预警 | 创建 alert_records |
| 复用 | 满足规则 + 已有未处理预警 | 跳过 |
| 自动关闭 | 不满足规则 + 有未处理预警 | is_resolved=true |
...
**库存扣减**：更新 `stocks.quantity` → 写入 `stock_versions`（change_type: "order"）→ 库存 ≤ 2 触发预警。
...
**订单**：`pending → assigned → partial_returned/returned → completed`（`cancelled` 可从 pending/assigned 进入）
